    ...
    <script type="text/javascript">
        ...
        function handleOptionChange() {
            ...
            if (selectedOption.val() === '') {
                ...
                if (<?php echo $config->showInventory ? 'true' : 'false'; ?>) { $('#inventory_display_' + itemId).html(''); }
                return;
            }
            ...
            // Update the name attribute to match the bundle and specific option combination
            if (bundleOptionId && bundleOptionId !== '') {
                quantitySelect.attr('name', itemId + '[' + bundleId + '][' + bundleOptionId + ']');
            } else {
            ...
            if (<?php echo $config->showInventory ? 'true' : 'false'; ?>) {
                updateInventoryDisplay(itemId, inventory);
            }
        }

        function updateInventoryDisplay(itemId, inventory) {
            $('#inventory_display_' + itemId).html('Inventory: ' + inventory);
        }
        ...
        function addToBasket(itemId, itemName, bundleId, bundleName, bundleOptionId, optionLabel, quantity, price) {
            // Check if this bundle already exists in the basket
            var existingItem = null;
            for (var i = 0; i < basket.length; i++) {
                if (basket[i].itemId === itemId && basket[i].bundleId === bundleId && basket[i].bundleOptionId === bundleOptionId) {
        ...
        function updateBasketDisplay() {
            ...
            basket.forEach(function (item) {
                ...
                if (item.optionLabel) { label += ' - ' + item.optionLabel; }
                basketHtml += label;
        ...
        function generateBasketFormFields() {
            ...
            basket.forEach(function (item) {
                var fieldName;
                if (item.bundleOptionId && item.bundleOptionId !== '') {
                    fieldName = item.itemId + '[' + item.bundleId + '][' + item.bundleOptionId + ']';
                } else {
                    fieldName = item.itemId + '[' + item.bundleId + ']';
                }
                $('<input>').attr({
                    type: 'hidden',
                    name: fieldName,
        ...
        $(document).ready(function () {
            ...
            $("body").delegate(".add-to-basket-btn", "click", function () {
                ...
                addToBasket(itemId, itemName, bundleId, bundleName, bundleOptionId, optionLabel, quantity, price);
